...
<div class="container">
    ...
    <div style="text-align: center; margin-bottom: 4rem;">
        ...
        <div style="font-size: 1.2rem; color: #4a5568; max-width: 800px; margin: 0 auto; text-align: center;">
            <ul style="list-style: none; padding: 0; display: flex; justify-content: center; gap: 2rem; flex-wrap: wrap;">
                <li style="display: flex; align-items: center;"><i class="fas fa-check-circle" style="color: #FFD700; margin-right: 0.5rem;"></i> Striving for Internationally Accredited Programs</li>
    ...
    <section style="margin-bottom: 5rem;">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(350px, 1fr)); gap: 2.5rem;">
            @forelse($undergraduatePrograms as $program)
            <div class="program-card" style="background: white; border-radius: 15px; overflow: hidden; box-shadow: 0 15px 30px rgba(10,35,66,0.1); transition: all 0.3s ease;">
                <div style="background: linear-gradient(135deg, #0a2342 0%, #1c3a5f 100%); padding: 2.5rem 2rem; text-align: center; color: white;">
                    <i class="{{ $program->icon ?? 'fas fa-laptop-code' }}" style="font-size: 3.5rem; margin-bottom: 1rem; color: #FFD700;"></i>
                    <h3 style="font-size: 1.8rem; font-weight: 600;">{{ $program->title }}</h3>
                </div>
                <div style="padding: 2.5rem;">
                    <p style="color: #4a5568; margin-bottom: 2rem; line-height: 1.8;">{{ $program->description }}</p>
                    ...
                    <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 0.5rem;">
                        ...
                        <span style="color: #4a5568;">✓ {{ $career }}</span>
                        @endforeach
                    </div>
                    @endif
                </div>
            ...
            <div style="text-align: center; padding: 3rem; background: white; border-radius: 15px;">
                <p>No undergraduate programs available.</p>
            </div>
            @endforelse
        </div>
    </section>

    <!-- Program Duration & Career Info Cards (Dynamic from first program) -->
    @php
        $firstProgram = $undergraduatePrograms->first();
    @endphp
    
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(350px, 1fr)); gap: 2.5rem; margin-bottom: 5rem;">
        ...
        <div class="program-card" style="background: white; border-radius: 15px; overflow: hidden; box-shadow: 0 15px 30px rgba(10,35,66,0.1); transition: all 0.3s ease;">
            ...
            <div style="padding: 2.5rem;">
                <div style="text-align: center; margin-bottom: 2rem;">
                    <span style="font-size: 4rem; font-weight: 700; color: #0a2342;">{{ $firstProgram->duration_years ?? 4 }}</span>
                ...
                <div style="background: #f8fafc; padding: 1.5rem; border-radius: 10px; margin-bottom: 1.5rem;">
                    <p style="margin-bottom: 0.8rem;"><strong style="color: #0a2342;">Total Credit Hours:</strong> {{ $firstProgram->credit_hours ?? 173 }}+ ECTS</p>
                    <p style="margin-bottom: 0.8rem;"><strong style="color: #0a2342;">Semesters:</strong> {{ $firstProgram->semesters ?? 8 }}</p>
                    <p style="margin-bottom: 0.8rem;"><strong style="color: #0a2342;">Study Mode:</strong> Full-time, Regular</p>
                    <p><strong style="color: #0a2342;">Teaching Method:</strong> {{ $firstProgram->teaching_method ?? 'Face-to-Face, Semester-based' }}</p>
                </div>
        ...
        <div class="program-card" style="background: white; border-radius: 15px; overflow: hidden; box-shadow: 0 15px 30px rgba(10,35,66,0.1); transition: all 0.3s ease;">
            ...
            <div style="padding: 2.5rem;">
                ...
                <div style="margin-top: 2rem; padding: 1rem; background: #e6f0fa; border-radius: 8px;">
                    <p style="color: #0a2342; text-align: center; font-weight: 500;">Top Employers: Software Companies, Government Organizations, Telecom Companies, Banks, IT Companies, and International Organizations</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <section style="margin-bottom: 5rem;">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(350px, 1fr)); gap: 2.5rem;">
            @forelse($postgraduatePrograms as $program)
            <div class="program-card" style="background: white; border-radius: 15px; overflow: hidden; box-shadow: 0 15px 30px rgba(10,35,66,0.1); transition: all 0.3s ease;">
                <div style="background: linear-gradient(135deg, #0f2c44 0%, #1e3a5a 100%); padding: 2.5rem 2rem; text-align: center; color: white;">
                    <i class="{{ $program->icon ?? 'fas fa-graduation-cap' }}" style="font-size: 3.5rem; margin-bottom: 1rem; color: #FFD700;"></i>
                    <h3 style="font-size: 1.8rem; font-weight: 600;">{{ $program->title }}</h3>
                </div>
                <div style="padding: 2.5rem;">
                    <p style="color: #4a5568; margin-bottom: 2rem; line-height: 1.8;">{{ $program->description }}</p>
                    ...
                    <div style="background: #f8fafc; padding: 1.5rem; border-radius: 10px; margin-bottom: 1.5rem;">
                        <p style="margin-bottom: 0.5rem;"><strong style="color: #0a2342;">Duration:</strong> {{ $program->duration_years ?? 2 }} Years</p>
                    ...
                    @if($program->specializations && count($program->specializations) > 0)
                    ...
                    <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 0.5rem;">
                        ...
                        <span style="color: #4a5568;">✓ {{ $specialization }}</span>
                        @endforeach
                    </div>
                    @endif
                </div>
            ...
            @empty
            <div style="text-align: center; padding: 3rem; background: white; border-radius: 15px;">
                <p>No postgraduate programs available.</p>
            </div>
            @endforelse
        </div>
    </section>
    ...
    <section style="background: linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%); padding: 4rem; border-radius: 20px; margin-bottom: 5rem;">
        ...
        <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 2rem; margin-bottom: 3rem;">
            <div style="text-align: center; padding: 2rem; background: white; border-radius: 15px; box-shadow: 0 10px 25px rgba(10,35,66,0.08);">
                ...
                <p style="color: #4a5568; font-size: 1.2rem;">{{ $firstProgram->duration_years ?? 4 }} Years (Regular)</p>
            ...
            <div style="text-align: center; padding: 2rem; background: white; border-radius: 15px; box-shadow: 0 10px 25px rgba(10,35,66,0.08);">
                ...
                <p style="color: #4a5568; font-size: 1.2rem;">{{ $firstProgram->mode_of_delivery ?? 'Face-to-Face' }}</p>
            ...
            <div style="text-align: center; padding: 2rem; background: white; border-radius: 15px; box-shadow: 0 10px 25px rgba(10,35,66,0.08);">
                ...
                <p style="color: #4a5568; font-size: 1.2rem;">{{ $firstProgram->teaching_method ?? 'Active Learning' }}</p>
        ...
        <div style="background: white; padding: 2.5rem; border-radius: 15px; margin-bottom: 2.5rem; box-shadow: 0 10px 25px rgba(10,35,66,0.08);">
            ...
            @forelse($admissionRequirements as $admission)
            <div style="background: #f0f5fa; padding: 2rem; border-radius: 12px; margin-bottom: 2rem;">
                <h4 style="color: #0a2342; margin-bottom: 1rem;">{{ $admission->title }}</h4>
                <p style="color: #4a5568; line-height: 1.8;">{{ $admission->content }}</p>
            </div>
            @empty
            <div style="background: #f0f5fa; padding: 2rem; border-radius: 12px;">
        ...
        <div style="background: white; padding: 2.5rem; border-radius: 15px; box-shadow: 0 10px 25px rgba(10,35,66,0.08);">
            ...
            <ul style="color: #4a5568; margin-bottom: 2rem;">
                @forelse($graduationRequirements as $grad)
                <li style="margin-bottom: 0.8rem; display: flex; align-items: center;"><span style="color: #FFD700; margin-right: 0.5rem;">✓</span> {{ $grad->requirement }}</li>
                @empty
                ...
                <table style="width: 100%; border-collapse: collapse; background: white; border-radius: 10px; overflow: hidden; box-shadow: 0 5px 15px rgba(0,0,0,0.05);">
                    ...
                    <tbody>
                        ...
                        <tr style="border-bottom: 1px solid #e2e8f0; {{ $loop->iteration % 2 == 0 ? 'background: #f8fafc;' : '' }}">
                            <td style="padding: 1.2rem; font-weight: 500;">{{ $req->category_name }}</td>
                            <td style="padding: 1.2rem; text-align: center;">{{ $req->number_of_courses }}</td>
                            <td style="padding: 1.2rem; text-align: center;">{{ $req->credit_hours }}</td>
                            <td style="padding: 1.2rem; text-align: center;">{{ $req->ects }}</td>
                        </tr>
                        @empty
                        ...
                        <tr style="background: #e6f0fa;">
                            ...
                            <td style="padding: 1.2rem; text-align: center; font-weight: 700; color: #0a2342;">{{ $totalCourses ?? 54 }}</td>
                            <td style="padding: 1.2rem; text-align: center; font-weight: 700; color: #0a2342;">{{ $totalCreditHours ?? 173 }}</td>
                            <td style="padding: 1.2rem; text-align: center; font-weight: 700; color: #0a2342;">{{ $totalEcts ?? 280 }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    <section style="text-align: center; margin-bottom: 3rem;">
        <div style="display: flex; justify-content: center; gap: 3rem; flex-wrap: wrap;">
            ...
            <div style="background: white; padding: 1rem 2rem; border-radius: 50px; box-shadow: 0 5px 15px rgba(0,0,0,0.05); display: flex; align-items: center;">
                ...
                <span style="color: #0a2342; font-weight: 500;">Ministry of Education Licensed</span>
            </div>
            <div style="background: white; padding: 1rem 2rem; border-radius: 50px; box-shadow: 0 5px 15px rgba(0,0,0,0.05); display: flex; align-items: center;">
